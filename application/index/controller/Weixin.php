...
class Index
{
    ...
    function getWeatherByCity()
    {
        $ch = curl_init();
        $city = '北京';
        $url = "http://apistore.baidu.com/microservice/weather?cityname=".$city;
        //3.设置参数
        curl_setopt($ch,CURLOPT_URL,$url);
        curl_setopt($ch,CURLOPT_RETURNTRANSFER,1);
        //3.调用接口
        ...
        if($res['errMsg'] == 'success'){
            $content = $res['retData']['weather']."\n".$res['retData']['city'];
        }
        //实例化模型
        $indexModel = new IndexModel;   //视频中封装好的sdk，本人没有做封装
        $indexModel -> responseText($postObj,$content);
    }
} //class end

//urlencode('北京');  //编码url字符串